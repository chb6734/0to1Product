...
6. ✅ **인증 보안 강화** - httpOnly 쿠키 기반 세션 관리 명시 (보안 요구사항 추가)
...
**현재 상태**: localStorage 사용 (보안 취약점 존재)

**권장 사항**: 
- **인증 토큰은 httpOnly 쿠키에 저장** (XSS 공격 방지)
- **사용자 프로필 정보는 localStorage에 저장 가능** (UI 표시용)
- **세션 관리는 서버 측에서 처리** (Supabase Auth 세션 활용)

**이유**:
1. **보안**: httpOnly 쿠키는 JavaScript로 접근 불가능하여 XSS 공격으로부터 안전
2. **자동 전송**: 쿠키는 모든 요청에 자동으로 포함되어 편리함
3. **만료 관리**: 서버에서 세션 만료 시간 관리 가능

**구현 방향**:
- Supabase Auth는 기본적으로 쿠키 기반 세션 관리 지원
- 로그인 성공 시 Supabase가 자동으로 httpOnly 쿠키 설정
- 프론트엔드에서는 사용자 프로필 정보만 localStorage에 저장 (UI 표시용)

**주의사항**:
- 민감한 정보(토큰, 세션 ID)는 절대 localStorage에 저장하지 않음
- 사용자 프로필 정보(nickname, profileImage 등)는 UI 표시를 위해 localStorage 사용 가능

---

### 2.3 온보딩 페이지 (`/onboarding`) - 개선
